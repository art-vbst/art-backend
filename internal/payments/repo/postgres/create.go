...
func (p *Postgres) CreateOrder(ctx context.Context, order *domain.Order) (*domain.Order, error) {
	...
	err := p.db.DoTx(ctx, func(ctx context.Context, q *generated.Queries) error {
		createOrderRow, err := q.CreateOrder(ctx, p.toDbCreateOrder(order))
		if err != nil {
			return err
		}

		createdOrder = p.toDomainCreateOrder(createOrderRow)
		return nil
	})
...
func (p *Postgres) toDbCreateOrder(order *domain.Order) generated.CreateOrderParams {
...
func (p *Postgres) toDomainCreateOrder(row generated.CreateOrderRow) *domain.Order {
	return &domain.Order{
		ID:              row.OrderID,
